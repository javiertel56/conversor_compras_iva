...
def convertir_excel(origen, destino):
    ...
    for v in agrupados.values():
        ...
        cuenta_str = str(v["cuenta"]).lower() if v["cuenta"] else ""
        # Omite si referencia contiene 'finiquito' o 'nomina'
        # O si concepto o nombre contiene 'sueldos y salarios'
        # O si la cuenta es de IVA o el concepto/nombre contiene 'iva'
        if (
        ...
        # Nueva lógica para cuentas 1104-
        ...
        # Si la referencia tiene IVA 8%, aplica 8% aunque no diga "8%" en el concepto
    ...
    with pd.ExcelWriter(destino, engine='openpyxl') as writer:
        pd.DataFrame(movimientos, columns=columnas).to_excel(writer, index=False, sheet_name='Sheet1')
        if hoja3:
            pd.DataFrame(hoja3, columns=columnas).to_excel(writer, index=False, sheet_name='Sheet3')
